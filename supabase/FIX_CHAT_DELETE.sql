...
-- Einmal im Supabase SQL-Editor ausführen (nach FIX_TEAM_CHAT.sql)
...
BEGIN
  ...
  -- Direktchat: Verlassen = für alle löschen
  ...
  -- Letztes Mitglied: Chat inkl. Medien entfernen (ohne Member-Check)
  IF remaining = 0 THEN
    SELECT coalesce(array_agg(a.storage_path), ARRAY[]::text[])
    INTO paths
    FROM public.chat_attachments a
