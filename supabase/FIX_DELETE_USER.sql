-- Nutzerkonto vollständig löschen (nur Admins; Cascades profiles via auth.users)
...
BEGIN
  ...
  -- Löscht auth.users → Cascades auf public.profiles
  DELETE FROM auth.users WHERE id = target_id;

  IF NOT FOUND THEN
    -- Fallback falls Auth-Zeile fehlt, Profil trotzdem entfernen
