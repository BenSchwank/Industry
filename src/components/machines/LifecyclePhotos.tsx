...
export function NotePhotoStrip({
  ...
  const deletePhoto = useDeleteNotePhoto()
  if (photos.length === 0) return null
      ...
      {photos.map((photo) => (
        <div key={photo.id} className="relative">
          <PhotoThumb
            photo={{
              id: photo.id,
              entry_id: photo.note_id,
              machine_id: photo.owner_id,
              storage_path: photo.storage_path,
              filename: photo.filename,
              mime_type: photo.mime_type,
              file_size_bytes: photo.file_size_bytes,
...
export function NotePhotoPicker({
  noteId,
  onUploaded,
}: {
  noteId: string
  ...
  const upload = useUploadNotePhotos()
  const [error, setError] = useState<string | null>(null)
  ...
  async function onFiles(list: FileList | null) {
    ...
    try {
      const files = [...list]
      for (const f of files) assertLifecycleImage(f)
      await upload.mutateAsync({ noteId, files })
      onUploaded?.()
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Upload fehlgeschlagen')
    }
